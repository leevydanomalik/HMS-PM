...
public class MovementSKHystrixCommandService {
    ...
    private GenericResponseDTO<MVSKChangeCommandDTO> defaultPutMovementSKFallback(MVSKChangeCommandDTO dto, Throwable e) throws IOException {
        return new GenericResponseDTO<MVSKChangeCommandDTO>().errorResponse(String.valueOf(HttpStatus.INTERNAL_SERVER_ERROR.value()),
                e instanceof HystrixTimeoutException ? "Connection Timeout. Please Try Again Later"
                        : e instanceof HystrixBadRequestException ? "Bad Request. Please recheck submitted data" : e.getLocalizedMessage());
    }
