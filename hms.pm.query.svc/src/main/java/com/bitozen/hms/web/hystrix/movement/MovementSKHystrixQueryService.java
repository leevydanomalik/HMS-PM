...
@Service
@Slf4j
public class MovementSKHystrixQueryService {
    ...
    @SneakyThrows
    @HystrixCommand(fallbackMethod = "defaultGetMovementSKByIDFallback")
    @Cacheable(value = "findOneByMovementSKIDCache", key = "#p0")
    public GenericResponseDTO<List<MovementSKDTOProjection>> getMovementSKByID(String skID) {
        try {
            CompletableFuture<List<MovementSKDTOProjection>> result = gateway.query(new GetMovementSKByIDQuery(skID), ResponseTypes.multipleInstancesOf(MovementSKDTOProjection.class));
            if(result.get() == null) {
    ...
    public GenericResponseDTO<List<MovementSKDTOProjection>> defaultGetMovementSKByIDFallback(String skID, Throwable e) throws IOException {
        return new GenericResponseDTO<List<MovementSKDTOProjection>>().errorResponse(String.valueOf(HttpStatus.INTERNAL_SERVER_ERROR.value()),
    ...
    public GenericResponseDTO<Map<String, Object>> getMovementForWeb(GetListRequestDTO dto) {
        try {
            CompletableFuture<List<MovementSKDTOProjection>> result = gateway.query(new GetAllMovementSKForWebQuery(dto), ResponseTypes.multipleInstancesOf(MovementSKDTOProjection.class));
            ...
            CompletableFuture<Long> count = gateway.query(new CountAllMovementSKForWebQuery(dto), Long.class);
